...
std::shared_ptr<EllipseShape> EllipseShape::Make(const double& start_pos_x, const double& start_pos_y, const double& radius_w, const double& radius_h) {
    return std::make_shared<EllipseShape>(start_pos_x, start_pos_y, radius_w, radius_h);
}
...
void EllipseShape::UpdateData(QPoint& click_point, QPoint& move_point) {
  ...
  /*  start_pos_x_ = double(min_x);
    ...
    radius_w_ = qAbs(double(dif.x()));
    radius_h_ = qAbs(double(dif.y()));
}
...
bool EllipseShape::EnterSelectRange(const QPoint& point) {
    const int kRangeSize = 30;
    QRectF outside_rectf{ start_point_.x() - kRangeSize, start_point_.y() - kRangeSize,  radius_w_ + 2 * kRangeSize, radius_h_ + 2 * kRangeSize };
    if (!PointInEllipse(outside_rectf, point)) {
        return false;
    }
    if (radius_w_ <= 2 * kRangeSize || radius_h_ <= 2 * kRangeSize) {
    ...
    QRectF inside_rectf{ start_point_.x() + kRangeSize, start_point_.y() + kRangeSize,  radius_w_ - 2 * kRangeSize, radius_h_ - 2 * kRangeSize };
    if (PointInEllipse(inside_rectf, point)) {
        return false;
    }
    return true;
...
void EllipseShape::DrawShape(QPainter& painter) {
    /*painter.drawEllipse(start_point_, radius_w_, radius_h_);*/


    painter.drawEllipse(QRectF(start_point_.x(), start_point_.y(), radius_w_, radius_h_));
}
...
void EllipseShape::PaintFrame(QPainter& painter) {
    ...
    QPen frame_pen(Qt::blue, 1, Qt::DashDotLine, Qt::RoundCap);
    painter.setPen(frame_pen);
    painter.drawRect(start_point_.x() - 5, start_point_.y() - 5, this->radius_w_ + 10, this->radius_h_ + 10);
    painter.restore();
}